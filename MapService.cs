...
namespace GameServer.Services
{
    class MapService : Singleton<MapService>
    {
        ...
        void OnMapTeleport(NetConnection<NetSession> sender,MapTeleportRequest request)//地图传送
        {
            Character character = sender.Session.Character;
            Log.InfoFormat("OnMapTeleport:characterID:{0}:{1} TeleporterId:{2}", character.Id, character.Data, request.teleporterId);
            
            if (!DataManager.Instance.Teleporters.ContainsKey(request.teleporterId))//校验传送点
            {
            ...
            //MapManager.Instance[source.MapID].CharacterLeave(sender, character);
